...
impl LogConfig {
    ...
    pub fn guard(&self) -> WorkerGuard {
        ...
        if self.format == FORMAT_PRETTY {
            let subscriber = subscriber.event_format(fmt_event.pretty());
            if self.stdout {
            ...
            let subscriber = subscriber.event_format(fmt_event.compact());
            if self.stdout {
                subscriber.with_writer(std::io::stdout).init();
            } else {
                subscriber.with_writer(file_writer).init();
            ...
            let subscriber = subscriber.event_format(fmt_event.json());
            if self.stdout {
            ...
            let subscriber = subscriber.event_format(fmt_event.with_level(self.with_level));
            if self.stdout {
                subscriber.with_writer(std::io::stdout).init();
            } else {
                subscriber.with_writer(file_writer).init();
